...
def load_data(filepath: str = 'data'):
    ...
    curated_train.to_csv('fashion-mnist_train_curated.csv', index=False)
    curated_test.to_csv('fashion-mnist_test_curated.csv', index=False)


def randomize_experiment(path='minus_plus_split.xlsx'):
    df = pd.read_excel(path, header=0)
    df = df.sample(frac=1)
    df.columns = ["id", "learning_rate", "batch_size", "layer_size", "layer_number", "response"]
    print("Randomized DataFrame:\n", df)
    df.to_csv('randomized_experiment.csv')
